...
let postTaxPrices = prices.map((currentVal) => {
    return currentVal * 1.07;
});
...
let totalPopulation = populations.reduce((previous, current) => {
...
let myStrongest = monstersInYourPocket.filter((currentVal, index, arr) => {
...
let ordersTotal = orders.map((currentVal, index) =>  {
...
let bobsTotal = purchases.reduce((previous, current, index, arr) => {
    var bobsPurchases;
    if (arr[index].owner === "Bob") {
        bobsPurchases = previous + current.price;
    } else {
        return previous;
    }
    return bobsPurchases;
}, 0);
